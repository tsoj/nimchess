...
1. e4 {Best by test} e5 {Solid reply} 2. Nf3 {Attacks e5} *
"""

    let games = readPgnFromString(pgnContent)
    check games.len == 1
    let game = games[0]
    check game.annotatedMoves.len == 3
    check game.annotatedMoves[0].annotation == "Best by test"
    check game.annotatedMoves[1].annotation == "Solid reply"
...
1. e4 (Main line) e5 2. Nf3 *
"""

    let games = readPgnFromString(pgnContent)
    check games.len == 1
    check games[0].annotatedMoves[0].annotation == "Main line"
...
"""
    ...
    let games = readPgnFromString(pgnContent)
    check games.len == 1
    check "First comment" in games[0].annotatedMoves[0].annotation
...
{This is a game overview} 1. e4 e5 *
"""

    let games = readPgnFromString(pgnContent)
    check games.len == 1
    check games[0].headers["PreMoveComment"] == "This is a game overview"
...
1. e4 {Great move} e5 {Solid} 2. Nf3 *
"""

    let games = readPgnFromString(pgnContent)
    let reparsed = games.toPgnString.readPgnFromString
    check reparsed.len == 1
    check reparsed[0].annotatedMoves[0].annotation == "Great move"
    check reparsed[0].annotatedMoves[1].annotation == "Solid"
...
"""
    ...
    let games = readPgnFromString(pgnContent)
    check games.len == 1
    for am in games[0].annotatedMoves:
      check am.annotation == ""
...
"""
    ...
    let games = readPgnFromString(pgnContent)
    check games.len == 1
    check games[0].headers["PreMoveComment"] == "Test game:"
    check games[0].annotatedMoves[0].annotation == "Scandinavian Defense:"
    check "Best" in games[0].annotatedMoves[2].annotation
  ...
  test "PGN annotation validation against ground truth":
    # The ground truth is generated by python-chess which strips RAV content from
    # annotations. Our parser captures ( ) content as the user requested, so our
    # annotations are a superset. We verify that every non-empty expected annotation
    # (from { } comments) is contained within our annotation.
    let games = readPgnFile("tests/testdata/pgns.pgn")
    let expectedLines = readFile("tests/testdata/pgns.annotations").splitLines()

    var lineIndex = 0
    for gameIndex, game in games:
      let gameNum = gameIndex + 1

      # Check pre-move comment
      if lineIndex < expectedLines.len:
        let expected = expectedLines[lineIndex]
        let prefix = "game " & $gameNum & " pre-move: "
        check expected.startsWith(prefix)
        let expectedComment = expected[prefix.len ..^ 1]
        let actual = game.headers.getOrDefault("PreMoveComment", "")
        if expectedComment.len > 0:
          check expectedComment in actual
        lineIndex += 1

      # Check per-move annotations
      for moveIndex, am in game.annotatedMoves:
        if lineIndex < expectedLines.len:
          let expected = expectedLines[lineIndex]
          let prefix = "game " & $gameNum & " move " & $moveIndex & ": "
          check expected.startsWith(prefix)
          let expectedAnnotation = expected[prefix.len ..^ 1]
          if expectedAnnotation.len > 0:
            check expectedAnnotation in am.annotation
          lineIndex += 1
